...
w1 = XYSupportNode(nr=1, glo_x=3.50, glo_y=0.00, glo_kx=kx1, glo_ky=ky1)
w2 = XYSupportNode(nr=2, glo_x=7.00, glo_y=4.00, glo_kx=kx2, glo_ky=ky2)
w3 = XYSupportNode(nr=3, glo_x=6.00, glo_y=5.00, glo_kx=kx3, glo_ky=ky3)
w4 = XYSupportNode(nr=4, glo_x=2.00, glo_y=4.00, glo_kx=kx4, glo_ky=ky4)

poly = Polygon([[0, 0], [7, 0], [7, 5], [0, 5]])

s1 = Stucture(xynodes=[w1, w2, w3, w4], glo_mass_centre=poly.centroid)

# s1.printTable()

load = Load(x_magnitude=1, y_magnitude=1)

ls1 = LinSolve(xy_structure=s1, xy_load=load)
